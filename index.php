...
while ($db_field = mysqli_fetch_assoc($result))
{
	...
	print "<tr><td><a href='services.php?device=" . $device . "&parent=" . $id . "&ip=" . $ip . "' alt='" . $ip . "'>" . $device . "</a></td><td>" . $info . "</td><td>" . $purpose . "</td><td class='on_off'>" . ($online ? 'online':'offline') . "</td><td><div class='percentbar' style='width:". round(100 * $scale) ."px;'><Center><font size='1'>" . round($percent * $scale) . "%<div style='width:" . round($percent * $scale) ."px;'></div></div></td></tr>";
}
